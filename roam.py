...
-------------------------------
* No wallet, no keys, no extensions in this browser at all.
...
def start_tunnel(port):
    ...
    A quick tunnel gives a public URL with no account; the address is random
    and changes every run, so it is only ever kept in memory and read back
    through /state - nothing hardcodes it anywhere.

    Needs the `cloudflared` binary on PATH; if it is missing this quietly
    no-ops and the server stays reachable on the local network only.
    """
    import re as _re
    import subprocess
    import threading
    ...
    if not CFD:
        say("no cloudflared on PATH - the public feed stays local-only")
        return

    proc = subprocess.Popen(
        [CFD, "tunnel", "--no-autoupdate", "--url",
    ...
    async with async_playwright() as pw:
        ...
        ctx = await browser.new_context(
            ...
            user_agent=("Mozilla/5.0 (Windows NT 10.0; Win64; x64) "
                        ...
                        "Chrome/124.0 Safari/537.36 openbrain/1.0"),
        )
...
def publish(stats, jpg, url, hz, neural=None):
    ...
    roaming itself - if nothing is reading these files, the fly does not care.
    """
    try:
